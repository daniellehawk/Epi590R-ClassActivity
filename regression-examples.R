...
poisson_model <- glm(nsibs~sex_cat + income + eyesight_cat,
										 data=nlsy, family=poisson())
tbl_regression(poisson_model, exponentiate = TRUE, label =list (sex_cat ~ "Sex",
																																eyesight_cat ~ "Eyesight",
																																income ~ "Income"
																																))
...
log_table <- tbl_regression(log_model,
														exponentiate = TRUE,
														label=list(eyesight_cat ~ "Eyesight",
																			 sex_cat ~ "Sex"))
...
tbl_merge(list(logistic_table, log_table),
					tab_spanner = c("**Logistic Regression**", "**Lo-linear Regression**"))
